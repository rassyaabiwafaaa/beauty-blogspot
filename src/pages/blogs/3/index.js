...
const Blog = () => {
  ...
  async function getData() {
    const response = await fetch("https://gorest.co.in/public/v2/posts?page=2&per_page=5");
  ...
  return (
    <>
      ...
      <div className="flex gap-3 lg:px-[100px] px-[40px] mb-10">
        <Link className="py-2 px-3 border-2 border-black text-black font-semibolds" href={"/blogs/2"}>
          {"<"}
        </Link>
        <Link href={"/blogs"} className="py-2 px-3 border-2 border-black text-black font-semibolds">
          1
        </Link>
        <Link href={"/blogs/2"} className="py-2 px-3 border-2 border-black text-black font-semibolds">
          2
        </Link>
        <Link href={"/blogs/3"} className="py-2 px-3 bg-primary text-white font-semibolds">
          3
        </Link>
      </div>
      <Footer />
